...
def update_coins(folder = 'price/', tail = '.csv'):
	...
	def download_coin_data(coin, folder='price/', tail='.csv'):
		'''
	...
	with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
		futures = [executor.submit(download_coin_data, coin) for coin in coins]
...
def blockchain_file_update(filename, url, folder='data/'):
	...
	TODO: Parse filename & url as dict/tuple to ensure pairing.
	'''
